...
"""SCAM verification for Ablation Test Case 2.1 using TACOT v2.2.

Same boundary conditions as TC 2.2 (rhoUeCH = 0.3 kg/m²/s, h_r = 1.5 MJ/kg)
but with char ablation disabled (B′_c = 0).  Wall enthalpy h_wall is still
read from the B′ table at each Newton iterate so that the SEB convective term

    q_conv = rhoUeCH_eff · (h_r − h_wall(T_w, p, B′_g))

uses the correct equilibrium wall gas enthalpy.  No recession occurs.

Reference: Amaryllis results from the Ablation Test Case series.

Verification status (open items — to be investigated later)
-----------------------------------------------------------
* SCAM slightly overpredicts the 98% pyrolysis front depth vs Amaryllis
  throughout the heating phase.  All other quantities (T_wall, m_dot_g,
  zero recession) are in good agreement.

Run from the repository root with::

    MPLBACKEND=Agg python3 \\
        examples/verification/Ablation_TestCase_2/Ablation_TestCase_2.1.py
"""
...
OUT_PNG = HERE / "Ablation_TestCase_2.1.png"
...
P_EDGE    = 101325.0
RHOUE_CH  = 0.3
H_RECOVERY = 1.5e6    # same as TC 2.2
...
class _NoCarbonAblationBackend:
    """Wrap a B′ backend to disable char mass loss while keeping h_wall.

    B′_c is forced to zero so that m_dot_char = 0 and no recession occurs.
    h_wall is still read from the underlying table (or live evaluator) so
    that the SEB convective term q_conv = rhoUeCH · (h_r − h_wall) uses
    the correct equilibrium wall gas enthalpy — matching TC 2.1's definition.
    """
    ...
    def lookup(self, T_wall, p_e, B_g, Z_C_pyro=None):
        _, h_wall = self._backend.lookup(T_wall, p_e, B_g, Z_C_pyro)
        return 0.0, h_wall  # B'_c = 0; h_wall from table
...
def make_bprime_backend(bprime_table):
    """Return a B′ backend with B′_c forced to zero (TC 2.1 definition).
    ...
    if BACKEND == "mpp":
        try:
            ...
            print("  Surface chemistry: Mutation++ (warm-start grid, B′_c = 0)")
            return _NoCarbonAblationBackend(ev)
    ...
    if BACKEND in ("mpp", "cantera"):
        try:
            ...
            print("  Surface chemistry: LIVE Cantera (B′_c = 0)")
            return _NoCarbonAblationBackend(ev)
        except Exception as exc:
            print(f"  WARNING: Cantera unavailable ({exc}); falling back to B′ table.")

    print("  Surface chemistry: pre-computed B′ table (B′_c = 0)")
    return _NoCarbonAblationBackend(bprime_table)
...
def load_reference_data() -> dict[str, np.ndarray]:
    return {
        "ama_energy": _load_reference("Amaryllis_Energy_TestCase_2.1.txt"),
        "ama_mass":   _load_reference("Amaryllis_Mass_TestCase_2.1.txt"),
    }
...
def run_scam():
    ...
    options = SolverOptions(
        ...
        output_dt=0.1,
        tc_positions=[],
        allow_recession=False,   # B'_c = 0 → no recession by definition
        continuous_remap=True,
        use_rho_old=True,
    )

    print(f"Running SCAM Ablation Test Case 2.1 with TACOT v2.2 [backend={BACKEND}] ...")
    return run(
        stack,
...
def make_plot(results, material, ref):
    ...
    fig.suptitle(
        "Ablation Test Case 2.1 — TACOT v2.2 (B′_c = 0, no char ablation)\n"
    ...
    # --- pyrolysis fronts (98% virgin only; no char front since B'_c = 0) ---
    ax = axes[1, 0]
    ...
    ax.set(title="Pyrolysis front (98% virgin, no char front)",
           xlabel="Time [s]", ylabel="Depth from original surface [mm]", xlim=(0, T_END))
    ax.legend(fontsize=8)
    ax.grid(alpha=0.25)

    # --- recession (zero by definition) ---
    ax = axes[1, 1]
    recession = results.s_array()
    ax.plot(t, recession * 1e3, color="black", lw=2.2, label="SCAM (= 0)")
    ax.plot(ama_m[:, 0], ama_m[:, 5] * 1e3, color="black", ls=":", lw=1.8,
            label="Amaryllis (= 0)")
    ax.axvline(60.0, color="0.4", ls=":", lw=1)
    ax.set(title="Surface recession (zero — no char ablation)",
           xlabel="Time [s]", ylabel="Recession [mm]", xlim=(0, T_END))
    ax.legend()
    ax.grid(alpha=0.25)
